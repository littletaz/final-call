...
/* A message can be one line or several. On a narrow screen a 9-slot line means
   tiles too small to read, so it breaks in two rather than shrinking — a board
   would stack its rows, not use a smaller typeface. */
export const SCRIPT = {
  ask:   { wide: 'ARE U IN?', narrow: ['ARE U', 'IN?'] },
  no:    { wide: 'FUCK*YOU!', narrow: ['FUCK', '*YOU!'] },
  taunt: { wide: 'U POOR?',   narrow: ['U', 'POOR?'] },
}

const NARROW = () => window.matchMedia('(max-width: 900px)').matches

/* a SCRIPT entry, or a plain string, resolved for this screen */
export function lines(message){
  if(typeof message === 'string') return [message]
  if(Array.isArray(message)) return message
  const picked = NARROW() ? message.narrow : message.wide
...
export class Wall {
  ...
  build(message = SCRIPT.ask, { silent = false } = {}){
    ...
    const rows_ = lines(message)
    const widest = Math.max(...rows_.map(l => l.length))
    ...
    if((cols - widest) % 2 !== 0) cols += 1
    /* an odd row count centres one line; an even one centres two */
    const wantOdd = rows_.length % 2 === 1
    if((rows % 2 === 0) === wantOdd) rows += 1

    this.cols = cols
    this.lineRows = rows_.map((_, i) =>
      Math.round((rows - rows_.length) / 2) + i)
    ...
    this.boards = Array.from({ length: rows }, (_, r) => {
      const line = this.lineRows.indexOf(r)
      const row = document.createElement('div')
      row.className = 'wall-row' + (line >= 0 ? ' is-message' : '')
      this.el.appendChild(row)

      if(line >= 0){
        const b = Flapboard.mount(row, silent ? ' ' : this.centred(rows_[line]), {
          length: cols, tick: 105, stagger: 46, cycles: 8, glyphs: { '*': FINGER },
        })
        this.messageBoards.push(b)
        this.messageBoard = this.messageBoards[0]
        return b
      }
  ...
  /* Resolves once every line has landed. With two lines they run together and
     the caller waits for the last. */
  say(message){
    const parts = lines(message)
    if(!this.messageBoards?.length) return Promise.resolve()
    return Promise.all(this.messageBoards.map((b, i) =>
      new Promise(resolve =>
        b.set(this.centred(parts[i] ?? ' '), { onSettle: resolve }))))
  }
